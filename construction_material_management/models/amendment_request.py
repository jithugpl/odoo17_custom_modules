...
class AmendmentRequest(models.Model):
    _name = 'amendment.request'
    _description = 'Amendment Request'
    ...
    amendment_request_line_ids = fields.One2many('amendment.request.line', 'line_id', string="Amendment Lines",
                                                 copy=False)
    ...
    note = fields.Html(string='Reason For Amendment')

    @api.model
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code('amendment.request.seq') or _('New')
        res = super(AmendmentRequest, self).create(vals)
        return res
    ...
    def action_select_all(self):
        for i in self.amendment_request_line_ids:
    ...
    def action_in_review(self):
        line_len = len([i for i in self.amendment_request_line_ids if i.is_select])
        line_qty = [i for i in self.amendment_request_line_ids if i.is_select and i.quantity <= 0]
        if not line_len:
            raise ValidationError('You need to select a line before Review')
        if len(line_qty):
            for i in line_qty:
                raise ValidationError(
                    _("The selected quantity must be greater than 0 for the product: %s" % i.product_id.name))
    ...
    def action_approve(self):
        ...
        if picking_type:
            picking = self.env['stock.picking'].sudo().create({
                ...
                'origin': 'Amendment Request %s' % self.name,
                'project_id': self.project_id.id,
                'task_id': self.task_id.id,
                'scheduled_date': self.expected_date,
                'amendment_request_id': self.id,
                'location_id': picking_type.default_location_src_id.id,
                'location_dest_id': picking_type.default_location_dest_id.id
            })
            for vals in self.amendment_request_line_ids:
                if vals.is_select and vals.quantity > 0:
                    picking.sudo().write({
    ...
    def action_cancel(self):
        picking = self.env['stock.picking'].sudo().search([('amendment_request_id', '=', self.id)])
    ...
    def button_open_picking(self):
        return {
            ...
            'view_mode': 'tree,form',
            'domain': [('amendment_request_id', '=', self.id)],
            'context': {'create': False, 'delete': False}
        }

    def _compute_transfer_count(self):
        for pick in self:
            picking = self.env['stock.picking'].sudo().search([('amendment_request_id', '=', pick.id)])
    ...
    def unlink(self):
        for rec in self:
            if rec.state == 'approved':
                raise ValidationError(
                    'You cannot delete a amendment request that in approved state.')
        return super(AmendmentRequest, self).unlink()


class AmendmentRequestLines(models.Model):
    _name = 'amendment.request.line'

    is_select = fields.Boolean(string="Select", default=True)
    line_id = fields.Many2one('amendment.request')
    product_id = fields.Many2one('product.product', string='Product')
    quantity = fields.Float(string="Qty", default=1.0)
    uom_id = fields.Many2one('uom.uom', string='UOM', compute='_compute_product_id_get_uom', store=True)
    is_true = fields.Boolean(string='Is True', compute='_compute_get_line_true')

    def _compute_get_line_true(self):
        for i in self:
            i.is_true = True if i.is_select and i.quantity > 0 else False

    @api.onchange('is_select', 'quantity')
    def _onchange_amendment_request_line_value(self):
        for i in self:
            i._compute_get_line_true()

    @api.depends('product_id')
    def _compute_product_id_get_uom(self):
        for i in self:
